...
<body>
  ...
  <script>
    ...
    function speakText(item, btn) {
      ...
      if (typeof config.rate === "number") u.rate = config.rate;
      if (typeof config.pitch === "number") u.pitch = config.pitch;
      btn.classList.add("playing");
    ...
    function handleClick(item, btn) {
      ...
      else if (item.text) { speakText(item, btn); }
