"""CRUD CLI for Azure Database for PostgreSQL (Flexible Server) with pgvector.

This script mirrors ``scripts/postgresql/crud.py`` but targets a managed
[Azure Database for PostgreSQL Flexible Server](https://learn.microsoft.com/en-us/azure/postgresql/)
instance with the ``pgvector`` extension enabled. It follows the
authentication pattern documented in the Microsoft Learn guide
...
Variable names are prefixed with ``AZURE_`` (e.g. ``AZURE_DBHOST``) to
distinguish them from the local pgvector ``POSTGRES_*`` variables.

Note on package choice
----------------------
The Microsoft Learn guide uses ``langchain-azure-postgresql`` (and
``AzurePGVectorStore``). At the time of writing that package pins
``pgvector>=0.4,<0.5`` while the ``langchain-postgres==0.0.17`` already used
by this project pins ``pgvector>=0.2.5,<0.4``, so the two cannot be installed
together. Because Azure Database for PostgreSQL Flexible Server is plain
PostgreSQL with the ``vector`` extension, we reuse ``langchain-postgres`` and
just supply an Azure-aware connection string (SSL required, Entra token used
as the password when ``AZURE_USE_ENTRA_AUTH=true``).
...
app = typer.Typer(
    ...
    help="Azure Database for PostgreSQL (pgvector) CRUD CLI",
)
    ...
    fake_embeddings: Annotated[
        ...
        typer.Option(
            ...
            help=(
                ...
                "Handy for end-to-end testing of the Azure connection path without "
                "having to call out to an embedding deployment."
            ),
        ),
    ] = False,
):
    """Azure PostgreSQL (pgvector) CRUD CLI - global options applied to every subcommand."""
...
def _resource_openai_v1_endpoint() -> str:
    ...
    Matches the helper in ``scripts/postgresql/crud.py``.
    """
...
def _resolve_db_password() -> tuple[str, str]:
...
@lru_cache(maxsize=1)
def _get_engine():
    """Return (and cache) a ``PGEngine`` connected to the Azure PostgreSQL server."""
    ...
    user, password = _resolve_db_password()
    url = settings.build_connection_string(password=password, user=user)
