"""Architecture tests for cloud_agent - verifies DDD layer boundaries."""
...
def test_application_has_no_infrastructure_imports() -> None:
    for file in _iter_python_files(BASE_PATH / "application"):
        imports = _imported_modules(file)
        assert not any(module.startswith("concierge.cloud_agent.infrastructure") for module in imports), file
