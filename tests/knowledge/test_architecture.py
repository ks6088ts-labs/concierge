...
BASE_PATH = Path("concierge/knowledge")
...
def test_application_has_no_framework_or_infrastructure_imports() -> None:
    for file in _iter_python_files(BASE_PATH / "application"):
        imports = _imported_modules(file)
        assert not any(module.split(".")[0] in FRAMEWORK_IMPORTS for module in imports), file
        assert not any(module.startswith("concierge.knowledge.infrastructure") for module in imports), file


def test_knowledge_module_does_not_import_agents() -> None:
    for file in _iter_python_files(BASE_PATH):
        imports = _imported_modules(file)
        assert not any(module.startswith("concierge.agents") for module in imports), file
